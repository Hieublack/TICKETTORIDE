...
def initEnv():
    ...
    for id_pl in range(NUMBER_PLAYER):
        ...
        # #print(env_state[ENV_IN4_PLAYER + ATTRIBUTE_PLAYER*id_pl : ENV_IN4_PLAYER + ATTRIBUTE_PLAYER*(id_pl + 1)])
    ...
    print(id_action, LIST_ALL_ROUTE_TEXT[np.where(env_state[ENV_ROUTE_CARD_GET : ENV_TRAIN_CAR_DROP] == 1)[0]], LIST_ALL_SCORE_ROUTE[np.where(env_state[ENV_ROUTE_CARD_GET : ENV_TRAIN_CAR_DROP] == 1)[0]])
...
def getAgentState(env_state):
    ...
    ### player_state[P_ID_PLAYER_END] = (env_state[ENV_ID_PLAYER_END] - id_action) % NUMBER_PLAYER
...
def action_player(env_state,list_player,file_temp,file_per):
    ...
    if getValidActions(player_state)[played_move] != 1:
        print(played_move,getValidActions(player_state) )
...
def check_winner(env_state):
    ...
    subtract_score = np.zeros(NUMBER_PLAYER)
    root_all_player_score = env_state[ENV_IN4_PLAYER : ENV_TRAIN_CAR_OPEN - 1 : ATTRIBUTE_PLAYER].copy()
    ...
    for player in range(NUMBER_PLAYER):
        ...
        for route_card in all_player_route_card:
            ...
            if check_done == 1:
                ...
                subtract_score[player] += LIST_ALL_SCORE_ROUTE[route_card]
                env_state[ENV_IN4_PLAYER + ATTRIBUTE_PLAYER * player : ENV_IN4_PLAYER + ATTRIBUTE_PLAYER* (player + 1)][0] -= LIST_ALL_SCORE_ROUTE[route_card]
    ...
    print('score', all_player_score, subtract_score, root_all_player_score)
    print('route_comp', number_route_comp)
    print('longest_path', list_longest_path)
    if len(winner_1) == 1:
        return winner_1[0]
    else:
        number_route_winner_comp = np.zeros(NUMBER_PLAYER)
        ...
        if len(winner_2) == 1:
            env_state[ENV_IN4_PLAYER + ATTRIBUTE_PLAYER * player : ENV_IN4_PLAYER + ATTRIBUTE_PLAYER* (player + 1)][0] += 0.1
            return winner_2[0]
        else:
            winner_3 = -1
            count = 0
...
def one_game(list_player, file_temp, file_per):
    env_state = initEnv()
    open_car = env_state[ENV_TRAIN_CAR_OPEN : ENV_ROUTE_CARD_GET].astype(np.int64)
    print('train_car_open = ', open_car, LIST_COLOR[open_car])
    route_card = env_state[ENV_ROUTE_CARD_BOARD : ENV_TRAIN_CAR_CARD].astype(np.int64)
    # print('ROUTE_CARD: ', route_card, LIST_ALL_ROUTE_TEXT[route_card[route_card > -1]])
    for player in range(NUMBER_PLAYER):
        player_att = env_state[ENV_IN4_PLAYER + player*ATTRIBUTE_PLAYER : ENV_IN4_PLAYER + (player + 1)*ATTRIBUTE_PLAYER]
        player_score = player_att[0]
        player_number_train = player_att[1]
        dict_tai_nguyen = {}
        for type_train in range(NUMBER_TYPE_TRAIN_CAR_CARD):
            player_att[2: 2 + NUMBER_TYPE_TRAIN_CAR_CARD]
            dict_tai_nguyen[LIST_COLOR[int(type_train)]] = player_att[2: 2 + NUMBER_TYPE_TRAIN_CAR_CARD][type_train]

        list_route = LIST_ALL_ROUTE_TEXT[np.where(player_att[2+ NUMBER_TYPE_TRAIN_CAR_CARD : ] > 0)]
        if player == 0:
            list_route = LIST_ALL_ROUTE_TEXT[np.where(env_state[ENV_ROUTE_CARD_GET : ENV_TRAIN_CAR_DROP] == 1)]
        route_score = LIST_ALL_SCORE_ROUTE[np.where(player_att[2+ NUMBER_TYPE_TRAIN_CAR_CARD : ] > 0)]
        print(f'Player {player}: điểm {player_score} số tàu {player_number_train}  có \n {dict_tai_nguyen} và {list_route}  {route_score}')
        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
    count_turn = 0
    while system_check_end(env_state) and count_turn < 7000:
        print('CHECKKKKKKKKKKKTURN', env_state[ENV_TURN], env_state[ENV_CHECK_END])
        count_turn += 1
        current_player = int(env_state[-2])
        player_state = getAgentState(env_state)
        action,file_temp, file_per = list_player[current_player](player_state,file_temp, file_per)
        print(f'action {action} @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@')
        if action < NUMBER_ROAD:
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} xây đường {LIST_ALL_ROAD_TEXT[action]}')
        if action in range(101, 147):
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} bỏ thẻ {LIST_ALL_ROUTE_TEXT[action - 101]}')
        if action in range(147, 157):
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} nhặt thẻ {LIST_COLOR[action - 147]}')
        if action in range(157, 167):
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} xây đường bằng màu {LIST_COLOR[action - 157]}')
        card_test_build_tunnel = np.where(env_state[ENV_CARD_TEST_TUNNEL : ENV_CARD_TEST_TUNNEL + NUMBER_TYPE_TRAIN_CAR_CARD].astype(np.int64) > 0)[0]
        if action == 167:
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} xác nhận xây hầm {LIST_ALL_ROAD_TEXT[int(env_state[ENV_ROAD_BUILT])]} vơi bộ test {LIST_COLOR[card_test_build_tunnel]}', env_state[ENV_CARD_TEST_TUNNEL : ENV_CARD_TEST_TUNNEL + NUMBER_TYPE_TRAIN_CAR_CARD][card_test_build_tunnel], env_state[ENV_CARD_TEST_TUNNEL : ENV_CARD_TEST_TUNNEL + NUMBER_TYPE_TRAIN_CAR_CARD])
        if action == 168:
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} xác nhận ko xây hầm {LIST_ALL_ROAD_TEXT[int(env_state[ENV_ROAD_BUILT])]} vơi bộ test {LIST_COLOR[card_test_build_tunnel]}', env_state[ENV_CARD_TEST_TUNNEL : ENV_CARD_TEST_TUNNEL + NUMBER_TYPE_TRAIN_CAR_CARD][card_test_build_tunnel], env_state[ENV_CARD_TEST_TUNNEL : ENV_CARD_TEST_TUNNEL + NUMBER_TYPE_TRAIN_CAR_CARD])
        if action == 169:
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} nhặt thẻ route')
        if action == 170:
            print(f'PHASE {int(env_state[ENV_PHASE])} Player {int(env_state[ENV_ID_ACTION])} dừng bỏ thẻ route')
        # print('TEST', env_state[ENV_PHASE], action, env_state[ENV_TRAIN_CAR_CARD: ENV_TRAIN_CAR_CARD + 5], env_state[ENV_TRAIN_CAR_OPEN : ENV_ROUTE_CARD_GET], env_state[ENV_TRAIN_CAR_DROP : ENV_CARD_BULD_TUNNEL])
        route_card = env_state[ENV_ROUTE_CARD_BOARD : ENV_TRAIN_CAR_CARD].astype(np.int64)
        print('ROUTE_CARD: ', route_card, LIST_ALL_ROUTE_TEXT[route_card[route_card > -1]])
        
        env_state = stepEnv(env_state, action)
        all_train_car = np.sum([np.sum(env_state[ENV_TRAIN_CAR_DROP:ENV_CARD_BULD_TUNNEL])
                    , np.sum(env_state[ENV_CARD_BULD_TUNNEL : ENV_PHASE])
                    , len(np.where(env_state[ENV_TRAIN_CAR_OPEN:ENV_ROUTE_CARD_GET] > -1)[0])
                    , len(np.where(env_state[ENV_TRAIN_CAR_CARD:ENV_IN4_PLAYER] > -1)[0])
                    , np.sum([np.sum(env_state[ENV_IN4_PLAYER + ATTRIBUTE_PLAYER * id_player_test : ENV_IN4_PLAYER + ATTRIBUTE_PLAYER* (id_player_test + 1)][2 : 2 + NUMBER_TYPE_TRAIN_CAR_CARD]) for id_player_test in range(NUMBER_PLAYER)])])
                        
        print('CHECK ALL TRAIN_CAR', all_train_car)
        if env_state[ENV_TRAIN_CAR_CARD] == -1 and np.sum(env_state[ENV_TRAIN_CAR_DROP : ENV_CARD_BULD_TUNNEL]) > 0:
            print(env_state[ENV_TRAIN_CAR_DROP : ENV_CARD_BULD_TUNNEL])
        ...
        print('train_car_open = ', open_car, LIST_COLOR[open_car])
        if action == 169:
            route_get = np.where(env_state[ENV_ROUTE_CARD_GET : ENV_TRAIN_CAR_DROP] > 0)[0]
            print('route car get ', LIST_ALL_ROUTE_TEXT[route_get],'point', LIST_ALL_SCORE_ROUTE[route_get])
        ...
        for player in range(NUMBER_PLAYER):
            ...
            for type_train in range(NUMBER_TYPE_TRAIN_CAR_CARD):
                ...
                dict_tai_nguyen[LIST_COLOR[int(type_train)]] = player_att[2: 2 + NUMBER_TYPE_TRAIN_CAR_CARD][type_train]
            list_route = LIST_ALL_ROUTE_TEXT[np.where(player_att[2+ NUMBER_TYPE_TRAIN_CAR_CARD : ] > 0)]
            print(f'Player {player}: điểm {player_score} số tàu {player_number_train} \n có {dict_tai_nguyen} và {list_route}')
            print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++')
...
def getValidActions(player_state_origin):
    ...
    # else:
    #     raise Exception('sai getValidActions', phase_game)
    if np.sum(list_action_return) == 0 and player_state[P_CHEKC_END] != 1:
    ...
    if phase_game == 1:
        ...
        if action < NUMBER_ROAD:    #nếu action là xây đường, xử lí xây đường
            ...
            if LIST_ALL_TYPE_ROAD[road] !=  2:
                ...
                # print('test', env_state[ENV_TYPE_TRAIN_CAR_BUILD_ROAD : ENV_TYPE_TRAIN_CAR_BUILD_ROAD + NUMBER_TYPE_TRAIN_CAR_CARD])
        ...
        #nếu nhặt thẻ locomotive thì update thông tin bàn chơi rồi chuyển người
            ...
            # print('TESTTTTTT', env_state[ENV_PHASE], action, env_state[ENV_TRAIN_CAR_CARD: ENV_TRAIN_CAR_CARD + 5], env_state[ENV_TRAIN_CAR_OPEN : ENV_ROUTE_CARD_GET], env_state[ENV_TRAIN_CAR_DROP : ENV_CARD_BULD_TUNNEL])
